...
- Full functional + RLS + negative-test pass in rolled-back transactions; `get_advisors(security)` clean (only the by-design SECURITY-DEFINER-RPC warnings remain). `docs/supabase-setup.md` updated as the schema source of truth. The Scope 1/2 open question is resolved (see Build decisions).
...
- Rename follow-up: the 0015 rename broke the **Emissions Platform** frontend (its deployed bundle still queries `ec_user_roles` by REST). Fixed with migration 0021 — a `security_invoker` compatibility **view** `ec_user_roles` → `user_roles` (RLS-enforced; verified admin=all, manager=own, anon=0; no new advisor lint). Bridge only: drop it once the Emissions Platform is migrated to `user_roles` and redeployed (its own repo, not this session's scope).
